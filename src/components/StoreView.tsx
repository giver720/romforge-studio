...
type DownloadEntry = { format: string; filename: string; url: string; size?: number | null };
type StoreEntry = {
  id: string;
  platforms: string[];
  name: string;
    ...
    <section className="scroll min-w-0 flex-1 p-6">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
          {filtered.map((entry) => {
            const download = entry.downloads[0];
            return <div key={entry.id} className="glass flex gap-3 rounded-xl p-3">
              ...
              <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><h2 className="truncate text-sm font-semibold">{entry.name}</h2><span className="shrink-0 text-[0.65rem] text-[var(--color-faint)]">{entry.platforms.map((p) => LABELS[p] ?? p.toUpperCase()).join(" · ")}</span></div><p className="mt-0.5 line-clamp-2 text-xs text-[var(--color-muted)]">{entry.summary || "Sin descripción"}</p><p className="mt-1 text-[0.65rem] text-[var(--color-faint)]">{entry.author || "Autor desconocido"}{entry.version ? ` · ${entry.version}` : ""}{download?.size ? ` · ${size(download.size)}` : ""}</p><div className="mt-2 flex gap-2"><button disabled={!download} onClick={() => download && openUrl(download.url)} className="btn btn-primary px-2.5 py-1 text-xs"><Download size={13} /> Descargar{download ? ` · ${download.format}` : ""}</button>{entry.release_url && <button onClick={() => openUrl(entry.release_url!)} className="btn btn-ghost px-2.5 py-1 text-xs"><ExternalLink size={13} /> Origen</button>}</div></div>
            </div>;
          })}
        </div>
      </div>
